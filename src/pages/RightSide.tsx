...
import { confirmOrdered } from "../store/slice/cart-slice";
import { ListOrder } from "../components/ListOrder";

export default function RightSide() {
  const dispatch = useDispatch()
  const { isActive, items, isConfirm } = useSelector((state: RootState) => state.cart);
    ...
    <div className="bg-white p-4  rounded-4 border border-light">
      <div>
        ...
        <div className="d-flex justify-content-between align-items-center py-2">
          ...
          <div className="text-sum-price">${(items.reduce((total, item) => total + item.total, 0)).toFixed(2)}</div>
        </div>
        <div className="d-flex gap-2 p-3 bg-custom1 rounded-3 my-3 justify-content-center align-items-center">
        ...
        <button className="btn-confirm-ordered" onClick={handleConfirmOrdered}>
          Confirm Order
        </button>
        </div>:
        <div className="d-flex flex-column align-items-center justify-content-center py-4 gap-3">
          ...
          <div className="text-category fw-bold">
